...
# =====================================================
st.set_page_config(page_title="ABSA – LDA + Long Short-Term Memory", layout="wide")
...
def load_sentiment_models():
    ...
    for aspek in ASPEK:
        ...
        model_path = os.path.join(SENT_MODEL_DIR, f"lstm_{key}.keras")
        tok_path = os.path.join(SENT_MODEL_DIR, f"tokenizer_{key}.pkl")
        ...
        if os.path.exists(model_path) and os.path.exists(tok_path):
            ...
            # Load tokenizer (.pkl)
            with open(tok_path, "rb") as fv:
                tok = pickle.load(fv)

            models[aspek] = (clf, tok)
...
def predict_sentiment_for_segment(seg_text: str, aspek: str, sent_models: dict, use_lexicon: bool = False):
    ...
    # pastikan aspek tersedia
    if aspek not in sent_models:
        return None, None

    model, tok = sent_models[aspek]  # model LSTM + tokenizer Keras

    # 1. Preprocess text
    clean_text = preprocess_for_sentiment(seg_text, use_lexicon=use_lexicon)
...
def main():
    ...
    if not sent_models:
        st.error("Model sentimen Long Short-Term Memory tidak ditemukan. Periksa folder MODEL.")
        st.stop()
    ...
    <div style="text-align:center; font-size:13px; color:gray; padding:10px;">
        © 2025 Akbar — Female Daily Review Analysis Dashboard  
    </div>
    """, unsafe_allow_html=True)
